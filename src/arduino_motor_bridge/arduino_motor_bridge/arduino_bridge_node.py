...
class ArduinoBridgeNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('connection_retry_seconds', 5.0,
                              ParameterDescriptor(type=ParameterType.PARAMETER_DOUBLE,
                                                 description='How often to retry connecting to Arduino if disconnected'))
        ...
        self.arduino.set_debug_callback(lambda msg: self.get_logger().info(f"Arduino Debug: {msg}"))
    ...
    def connect_to_arduino(self):
        """Try to connect to the Arduino and set up retry timer if it fails."""
        if not self.arduino.connect():
            self.get_logger().error("Failed to connect to Arduino. Will retry in {} seconds.".format(
                self.connection_retry_seconds))
            
            # Create a timer to retry the connection
            if self.connection_retry_timer is None:
                self.connection_retry_timer = self.create_timer(
                    self.connection_retry_seconds, self.retry_arduino_connection)
        else:
            # If we successfully connected, cancel any retry timer
            if self.connection_retry_timer is not None:
                self.connection_retry_timer.cancel()
                self.connection_retry_timer = None
                
    def retry_arduino_connection(self):
        """Retry connecting to the Arduino."""
        self.get_logger().info("Retrying connection to Arduino...")
        self.connect_to_arduino()
