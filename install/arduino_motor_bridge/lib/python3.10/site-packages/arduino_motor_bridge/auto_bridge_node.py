...
# Import transforms3d for quaternion operations
from transforms3d.euler import euler2quat

from arduino_motor_bridge.auto_arduino_serial import AutoArduinoSerial

class AutoArduinoBridge(Node):
    """
    Bridge between ROS2 and Arduino with automatic port detection and reconnection.
    ...
    def __init__(self):
        ...
        self.declare_parameter('connection_retry_seconds', 5.0)  # seconds
        ...
        self.connection_retry_seconds = self.get_parameter('connection_retry_seconds').value
        ...
        # Create the auto-connecting serial interface
        self.arduino = AutoArduinoSerial(
        ...
        if not self.arduino.connect(retry_count=3):
            self.get_logger().error("Failed to connect to Arduino. Node will continue and retry automatically.")
        ...
        # Timer to ensure odom->base_link transform is published even before receiving real odometry
        self.tf_timer = self.create_timer(0.1, self._publish_static_tf)
        
        # Flag to track if we've received real odometry data
        self.received_odom_data = False
        
        self.get_logger().info("Arduino Bridge Node initialized with auto-detection")
        
    def _publish_static_tf(self):
        """
        Publish a static transform from odom to base_link if no real odometry is available.
        This ensures the navigation stack can find the relationship between these frames.
        """
        if not self.received_odom_data:
            # Publish identity transform (no motion)
            t = TransformStamped()
            t.header.stamp = self.get_clock().now().to_msg()
            t.header.frame_id = "odom"
            t.child_frame_id = "base_link"
            
            # Set transform (identity - no movement)
    ...
    def _handle_odometry(self, left_pos, right_pos, left_dist, right_dist):
        ...
        """
        # Mark that we have received real odometry data
    ...
    def _publish_odom_tf(self, now):
        ...
        # Convert rotation from yaw to quaternion
    ...
    def shutdown(self):
        ...
        """
        self.get_logger().info("Shutting down Auto Arduino Bridge Node")
